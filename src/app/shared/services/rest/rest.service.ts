...
export class RestService {
  ...
  public deleteServiceWithId(url: string, key: string, val: string): Observable<any> {
    /// show loader
    return this.http
      .delete(this.appUrl + url + '/?' + key + '=' + val, this.options)
      .pipe(
        map(this.fromResponse),
      catchError(this.catchServerError),
      finalize(()=>{
  ...
  public patchService(url: string, param: any): Observable<any> {
    ...
    //show loader
    return this.http
      .patch(this.appUrl + url, body, this.options)
      .pipe(
        map(this.fromResponse),
      catchError(this.catchServerError),
      finalize(()=>{
...
//Data from Response
  ...
  public catchServerError(errors: any) {    
    let errorMsg = errors.message || 'Something went wrong on Server!'; 
     if (errors) {
